...
int classify2(const vector<GMM> &vgmm, const fmatrix &dat, const vector<GMM> &vgmmd, const fmatrix &datd, const vector<GMM> &vgmmdd, const fmatrix &datdd, float &maxlprob) {
  float  lprob;
  int maxind  = -1;
  maxlprob = -1e38;
  ...
  maxind = 0;
	for(unsigned int i=0; i<vgmm.size() && i<vgmmd.size() && i<vgmmdd.size(); i++){
		lprob = vgmm[i].logprob(dat) + vgmmd[i].logprob(datd) + vgmmdd[i].logprob(datdd);
		if(lprob>maxlprob){
			maxlprob = lprob;
			maxind = i;
		}
...
/*
	int retv0 = read_options(argc, argv, input_dirs, input_exts, 
			  gmm_dirs, gmm_exts, input_filenames, gmm_filenames);
...
*/
  ...
  */
  ...
  vector<GMM> vgmm;
  int retv0 = read_gmms(gmm_dirs[0], gmm_exts[0], gmm_filenames, vgmm);
	vector<GMM> vgmmd;
  int retv1 = read_gmms(gmm_dirs[1], gmm_exts[1], gmm_filenames, vgmmd);
	vector<GMM> vgmmdd;
  int retv2 = read_gmms(gmm_dirs[2], gmm_exts[2], gmm_filenames, vgmmdd);
  if (retv0 != 0 || retv1 != 0 || retv2 != 0)
    return usage(argv[0], retv);

  ///Read and classify files

  for (unsigned int i=0; i<input_filenames.size(); ++i) {
    fmatrix dat, datd, datdd;
    string path0 = input_dirs[0] + input_filenames[i] + input_exts[0];
		string path1 = input_dirs[1] + input_filenames[i] + input_exts[1];
		string path2 = input_dirs[2] + input_filenames[i] + input_exts[2];
    ifstream ifs(path0.c_str(), ios::binary);
    if (ifs.good())
      ifs >> dat;
		ifstream ifsd(path1.c_str(), ios::binary);
    if (ifsd.good())
      ifsd >> datd;
		ifstream ifsdd(path2.c_str(), ios::binary);
    if (ifsdd.good())
      ifsdd >> datdd;

    if (!ifs.good() || !ifsd.good() || !ifsdd.good()) {
      cerr << "Error reading data file: " << path0 << endl;
      return usage(argv[0],1);
    }

    int nclass;
    float lprob;
    nclass = classify2(vgmm, dat, vgmmd, datd, vgmmdd, datdd, lprob);
